/*
 * Modified version of `androidx.compose.ui.graphics.Matrix.kt`
 * - Replaced references to Compose classes with our own equivalents:
 *   - MutableRect -> MutableRectF
 *   - Rect -> RectF
 *   - Offset -> FloatPoint
 *
 * - Replaced references to Compose utility functions with standard functions
 *   - fastIsInfinite -> isInfinite
 *   - fastMinOf -> minOf
 *   - fastMaxOf -> maxOf
 *
 * Original license:
...
value class Matrix(
    ...
    /** Does the 3D transform on [point] and returns the `x` and `y` values in an [FloatPoint]. */
    fun map(point: FloatPoint): FloatPoint {
        ...
        val inverseZ = 1 / z
        val pZ = if (inverseZ.isInfinite()) inverseZ else 0f

        return FloatPoint(x = pZ * (v00 * x + v10 * y + v30), y = pZ * (v01 * x + v11 * y + v31))
    }
    ...
    fun map(rect: RectF): RectF {
        ...
        var pZ = if (inverseZ.isInfinite()) inverseZ else 0f
    ...
    fun map(rect: MutableRectF) {
        ...
        rect.left = minOf(x0, x1, x2, x3)
        rect.top = minOf(y0, y1, y2, y3)
        rect.right = maxOf(x0, x1, x2, x3)
        rect.bottom = maxOf(y0, y1, y2, y3)
    }

    inline fun map(out: Matrix, transform: (index: Int, value: Float) -> Float): Matrix {
        for (index in values.indices) {
            out.values[index] = transform(index, values[index])
        }
        return out
    }
    ...
    inline fun build(block: Matrix.() -> Unit): Matrix = this.apply {
        reset()
        block()
    }

    fun copy(): Matrix = Matrix(values)
}
